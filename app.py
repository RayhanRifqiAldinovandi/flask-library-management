...
def index():
    conn = db_connection()
    cur = conn.cursor()
    sql = """SELECT bor.borrow_id, usr.name FROM users usr JOIN borrowed bor ON bor.user_id = usr.user_id"""
    cur.execute(sql)
...
def read(borrowed_id):
    ...
    cur = conn.cursor()
    sql = """
        SELECT bor.borrow_id, bor.book_title, bor.date_borrowed, bor.date_due, bor.days_of_loan FROM borrowed bor JOIN users usr ON usr.user_id = bor.user_id WHERE bor.borrow_id = %s
    """ % borrowed_id
    cur.execute(sql)
...
def edit(borrowed_id):
    ...
    conn = db_connection()
    cur = conn.cursor()
    sql = 'SELECT bor.borrow_id, bor.book_title, bor.date_borrowed, bor. date_due, bor. days_of_loan FROM borrowed bor WHERE borrow_id = %s' % borrowed_id
    cur.execute(sql)
    borrowed = cur.fetchone()
    cur.close()
    conn.close()
...
def delete(borrow_id):
    ...
    if request.method == 'POST':
      ...
      conn = db_connection()
      cur = conn.cursor()
      sql = 'DELETE FROM borrowed WHERE id = %s' % borrow_id
      cur.execute(sql)
      cur.close()
      conn.commit()
      conn.close()
...
def add():
     if request.method == 'POST':
      ...
      db = db_connection()
      cur = db.cursor()
      sql = ("""INSERT INTO books (book_title,author,genre) VALUES ('%s','%s','%s')""" % (book_title,author,genre))
      cur.execute(sql)
      db.commit()
      cur.close()
      db.close()
